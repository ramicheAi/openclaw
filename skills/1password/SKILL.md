---
name: 1password
description: Set up and use 1Password CLI (op). Use when installing the CLI, enabling desktop app integration, signing in (single or multi-account), or reading/injecting/running secrets via op. Handles live secrets — never print, log, paste into messages, or send secret values off the machine; inject them only into the command the user asked to run.

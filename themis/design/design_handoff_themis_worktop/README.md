...
In addition, the original repo files that the design extends:
- `themis/src/index.css` — design tokens (already in repo)
- `themis/src/types.ts` — data shapes (already in repo)
- `themis/src/lib/api.ts` — typed API client (already in repo)
- `themis/src/lib/ui.tsx` — atom components (already in repo)
